...
El retrato del elenco es **la foto de la clinica entera**: ni recorte, ni mascara, ni escala. Hasta que llegaron esas fotos era un recorte fingido —una mascara ovalada que apagaba el fondo hacia los cantos, la figura escalada un 20% con el origen a la altura de la cara y el marco comiendose lo que sobraba—, y hacia falta porque las fotos de archivo no venian encuadradas para esto; las de la clinica si: la persona ya viene centrada sobre el consultorio. El elenco reparte la pantalla en dos, y **cual piedra va en cada lado no es indiferente**: el marmol debajo del profesional y la madera debajo de lo que se cuenta de el. De paso la mitad oscura es la del texto, que es donde la ficha ya era negra. **Dentro de la escena, en escritorio, la foto ES la mitad**: de canto a canto y de arriba abajo, hasta la juntura de oro. El aire no desaparece, cambia de sitio —lo pone la columna de la derecha, que es la que lleva texto—, y arriba la foto se mete por debajo de la barra fija para que la cabeza caiga siempre por debajo de su canto, mida lo que mida la ventana. Aqui la foto llena y el marco recorta, que es lo contrario de lo que hace apilada: el hueco es casi el de la foto —0,86 contra 0,75 de la mas estrecha— asi que lo que se va son unos dedos de cintura, y el `object-position` al 20% reparte ese sobrante dejando la cabeza donde estaba. Las flechas se apoyan encima, que ya no hay canto libre donde ponerlas. Fuera de la escena —con el movimiento reducido— el marco sigue midiendo `min(80svh, 820px)` con la foto entera dentro. **Apilado, el retrato es la mitad de arriba de la pantalla** y la ficha la de abajo; ahi la juntura de las dos piedras no va a ojo, se calcula (`var(--alto-navbar) + … + var(--alto-retrato) + .85rem`), porque el retrato mide en `svh` y la ficha mide lo que mide su texto: con un porcentaje fijo, en una tableta la foto cruzaba la juntura y en un telefono corto la juntura le caia dos dedos por debajo. Por lo mismo el bloque se arrima al techo en vez de centrarse: la cuenta sale del techo. En la **placa** (`.ns-elenco__placa`), apoyada sobre el pecho y no colgando del marco, va solo la especialidad —el nombre era de relleno—, y hace falta desde que el retrato vive sobre marmol: iba en blanco porque debajo tenia madera.
