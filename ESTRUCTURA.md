...
| `10-footer.css` | Pie de página, mapas, formulario, redes y los dos botones flotantes (WhatsApp y subir). La marca del pie es el mismo logo que el de la barra (`.marca-logo`, a 72px de alto). Las cabeceras del pie («Ubícanos», «Escríbenos») van en su caja de oro, como las de cada sección; la rayita de oro que llevaban debajo se fue con ella, que debajo de un recuadro con filo de oro otra raya es repetir. | `.footer*`, `.mapa-*`, `.redes-sociales`, `.wsp-flotante`, `.btn-subir` |
...
El que **se queda** es `imagenes/logo-smilers-marmol.webp` (y su versión de
440px), el sello de Especialidades, que va sobre mármol claro. Sale de aquel
recorte, con el negro pasado a transparente y lo que era plata o blanco
pasado a tinta `#2B2B2B`: con el archivo nuevo tal cual, sobre ese mármol el
diente y el «Dental Clinique» se perderían. Si la clínica pasa algún día una
versión para fondo claro, ese es el sitio donde va.
