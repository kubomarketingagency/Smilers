...
| `27-consentimiento.css` | **Va la última y la cargan las seis páginas.** El aviso de cookies vestido de la casa —negro de ónix, filo y botones de oro, la letra del sitio— a través de las variables de CookieConsent, con `:root #cc-main` para ganarle a su hoja, que llega después. En el teléfono pone aceptar y rechazar en una fila (la librería los apila con `!important`). Y los dos enlaces legales del pie: «Política de privacidad» y el botón «Preferencias de cookies», que nace con `hidden` y solo enciende el guion cuando hay píxeles configurados. | `#cc-main`, `.footer-legal`, `.enlace-cookies` |
...
cookies, ni botón de «Preferencias de cookies» en el pie, ni una sola petición
a Meta o a Google. Para encenderlos:
...
- **Ningún píxel está en el HTML.** El código de Meta y el de Google se
  escriben en la página solo cuando alguien pulsa «Aceptar» (`activarPixeles()`).
  Rechazar, cerrar el aviso o no contestar es no cargar nada. Se comprobó con
  Chrome bloqueando y apuntando toda petición a Meta y Google: cero antes de
  aceptar, las dos justo después, y cero después de rechazar.
...
- **Aceptar y rechazar pesan lo mismo** (`equalWeightButtons`), y en el
  ...
  logo de la portada.
- **Retirar el permiso** (desde «Preferencias de cookies») borra `_fbp`,
  `_fbc` y `_gcl_*` del dominio y recarga la página, que es la única forma de
  descargar los guiones de Meta y Google.
