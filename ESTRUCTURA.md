...
| `imagenes/Carta de Bienvenida/` | La carta y el retrato **tal cual los pasó la clínica**, como fuente. No se publica (`.vercelignore`): la carta está rehecha en HTML —a 320×455 la imagen no da para media pantalla— y de ella solo sale la firma. | no |
...
| `29-carta.css` | **Va la última y solo la carga Nosotros.** La carta de bienvenida: la tercera escena clavada de esa página, entre el cine y el cierre de cifras, montada igual que el cierre —300vh de envoltorio con un pin de `100lvh` dentro y un margen negativo de una pantalla, para engancharse a la última del cine sin dejar en medio una pantalla de negro quieto—. **Dos coreografías con el mismo progreso, y eso lo hace esta hoja, no el guion**: en pantalla ancha las dos mitades conviven a partes iguales —el director a la izquierda, la carta a la derecha— y se descubren por orden, las dos de izquierda a derecha (`clip-path: inset`), con la juntura de oro dibujándose de arriba abajo entre ellas; en el teléfono no caben las dos, así que se turnan —el retrato ocupa la pantalla con la frase encima y el papel sube desde abajo y lo tapa—, y la frase no necesita irse por su cuenta porque la tapa el propio papel. El papel es crema editorial con una veladura cálida, filo de oro por dentro del canto, **cuatro escuadras que se dibujan solas** (`--k-filo`) y un lustre que lo cruza una vez cuando acaba de posarse; en ancho no va a sangre, sino apoyado sobre el negro con aire y sombra, que es lo que lo hace leerse como una hoja de papel y no como media pantalla de color. La firma se escribe de izquierda a derecha con `--k-firma`, que es lo último que mueve la escena. Las `--k-*` van registradas con `@property` **y aquí sí heredan**, al revés que las del cine: allá el guion las escribía en la escena entera y cortar la herencia ahorraba recalcular doscientos elementos; aquí cada una se escribe ya en la pieza pequeña que la usa, y entre lo que cuelga de esa pieza hay pseudo-elementos —la juntura, las ocho rayas de las escuadras— que tienen que leerlas. Su valor inicial es 1, el estado final: sin guion la carta se ve entera. En el teléfono el marco del papel arranca por debajo de la barra de navegación, que es fija y opaca y se comía el filo y las dos escuadras de arriba. | `.ns-carta*`, `--k-*` |
