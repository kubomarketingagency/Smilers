...
| Archivo | Qué hace |
|---|---|
| `planificador.js` | El planificador único de scroll (`SmilersScroll`). Un solo listener para todos los efectos, con las lecturas y las escrituras separadas. Y los deslizamientos (`deslizarA(destino, duración, arranque, alLlegar)`): sin `arranque` salen y llegan despacio, como un ancla; con `arranque` (la velocidad de salida en px/ms) salen ya en marcha y frenan hasta pararse, que es lo que pide un gesto. `velocidad()` y `destino()` dicen cómo va el que está en curso, para que otro lo releve sin que se note. El dedo corta un deslizamiento con `touchstart` y no con `pointerdown`, que llega antes y no dejaba ver adónde iba. |
