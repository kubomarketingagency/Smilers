...
| `29-carta.css` | **Va la última y solo la carga Nosotros.** La carta de bienvenida: la tercera escena clavada de esa página, entre el cine y el cierre de cifras, montada igual que el cierre —300vh de envoltorio con un pin de `100lvh` dentro y un margen negativo de una pantalla, para engancharse a la última del cine sin dejar en medio una pantalla de negro quieto—. **Dos coreografías con el mismo progreso, y eso lo hace esta hoja, no el guion**: en pantalla ancha las dos mitades conviven a partes iguales —el director a la izquierda, la carta a la derecha— y se descubren por orden, las dos de izquierda a derecha (`clip-path: inset`), con la juntura de oro dibujándose de arriba abajo entre ellas; en el teléfono no caben las dos, así que se turnan —el retrato ocupa la pantalla con la frase encima y el papel sube desde abajo y lo tapa—, y la frase no necesita irse por su cuenta porque la tapa el propio papel. **Lo que va dentro de la hoja es el documento que pasó la clínica, rehecho pieza por pieza** (`.cb-*`): los dos arcos de su esquina izquierda salen del original —medidos píxel a píxel y pasados a curvas cuadráticas, unos 4,5 KB de `path`— y cada bloque está en el porcentaje exacto en que estaba, con la letra en `cqw` (tanto por ciento del ancho de la hoja), así que la hoja escala de una pieza igual que escalaría la imagen. **No se usa el PNG original** porque mide 320×455: a media pantalla su letra de cuerpo mediría seis píxeles y se vería como la foto de un papel. Rehecho se lee a cualquier tamaño, se puede seleccionar y copiar, lo indexa un buscador y lo lee en voz alta un lector de pantalla; del original solo sobrevive la firma (`imagenes/carta/firma-director.png`, recortada y con alfa), que se escribe de izquierda a derecha con `--k-firma` al final de la escena. La única diferencia con el papel es la tipografía: el documento va en una geométrica y aquí va la Lato del sitio, porque una tercera familia por una sección no se paga (lo mismo que se decidió al quitar Prata). **El ancho de la hoja sale del alto y no al revés** —`min(100%, (100lvh - la barra - el aire) × 320/455)`—: una hoja A4 en una pantalla apaisada la limita el alto, y escrito al revés (un `aspect-ratio` con `max-height`) el navegador encoge un lado, deja el otro y la hoja sale deformada. La barra de navegación se descuenta siempre, no solo en el teléfono, porque es fija y opaca en las dos. Alrededor del documento, las **cuatro escuadras de oro** (`--k-filo`) que lo enmarcan por fuera, y un **lustre** que lo cruza una vez al posarse. Las `--k-*` van registradas con `@property` **y aquí sí heredan**, al revés que las del cine: allá el guion las escribía en la escena entera y cortar la herencia ahorraba recalcular doscientos elementos; aquí cada una se escribe ya en la pieza pequeña que la usa, y entre lo que cuelga de ella hay pseudo-elementos —la juntura, las ocho rayas de las escuadras— y los bloques del documento, que tienen que leerla. Su valor inicial es 1, el estado final: sin guion la carta se ve entera. **Lo impreso cuelga de una sola pieza** (`.cb-tinta`, del tamaño exacto del documento) y es esa pieza la que se funde: antes se fundían a la vez los diecisiete bloques, diecisiete planos que componer en cada fotograma de ese tramo, y en un teléfono eso se nota. Por eso `--k-texto` es **la única `--k-*` que no hereda**: se escribe en `.cb-tinta` y la usa ella sola, así que cambiarla no obliga a repasar el estilo de los cuarenta nodos que lleva dentro. **Los datos del documento llevan a donde llevan**: los tres teléfonos llaman, los tres iconos y la arroba abren su perfil y las dos direcciones abren el mapa. Se ven exactamente igual que cuando eran texto muerto —ni color distinto ni subrayado, que lo que se enseña es la carta de la clínica y no una página de enlaces— y solo responden cuando el documento está a pantalla completa, que es cuando el pin deja pasar el dedo. **Y mientras la carta ocupa la pantalla, los tres botones flotantes se apartan** (`.con-carta` en el `<html>`, que pone `nosotros-cine.js`): en el teléfono el documento va de canto a canto y les caía encima —el de cookies, justo sobre el nombre del director— y no hay sitio al que moverlos, porque lo que sobra ahí es el negro de arriba y de abajo. Se van con un fundido y vuelven solos en cuanto el cierre se clava encima. | `.ns-carta*`, `.cb-*`, `.con-carta`, `--k-*` |
