...
| `29-carta.css` | **Va la última y solo la carga Nosotros.** La carta de bienvenida: la tercera escena clavada de esa página, entre el cine y el cierre de cifras, montada igual que el cierre —300vh de envoltorio con un pin de `100lvh` dentro y un margen negativo de una pantalla, para engancharse a la última del cine sin dejar en medio una pantalla de negro quieto—. **Dos coreografías con el mismo progreso, y eso lo hace esta hoja, no el guion**: en pantalla ancha las dos mitades conviven a partes iguales —el director a la izquierda, la carta a la derecha— y se descubren por orden, las dos de izquierda a derecha (`clip-path: inset`), con la juntura de oro dibujándose de arriba abajo entre ellas; en el teléfono no caben las dos, así que se turnan —el retrato ocupa la pantalla con la frase encima y la carta sube desde abajo y lo tapa—, y la frase no necesita irse por su cuenta porque la tapa la propia carta. **Lo que se enseña es el contenido de la carta, no el papel.** Hubo una temporada en que aquí se rehacía el documento entero de la clínica —su banda negra con los arcos de oro, su columna de teléfonos, redes y direcciones, su raya vertical, su hoja blanca en proporción A4—. De ese documento se queda **lo que dice**; el resto se fue, y con la columna se fueron también sus enlaces (el teléfono, las redes, los mapas), que el pie lleva enteros: eran los mismos datos tres veces en la misma página. La hoja blanca además obligaba a dejar dos franjas de negro arriba y abajo, porque una hoja vertical dentro de una pantalla más estrecha se queda sin alto antes que sin ancho. Ahora la carta es **una plancha de ónix que ocupa su mitad entera** (`.ns-carta__panel`), con el logo de oro de la casa arriba —el de la muela de plata, el mismo de la barra y del hero, no el gris del documento: sobre ónix ese es el que vive—, el texto debajo y las **cuatro escuadras de oro** (`--k-filo`) dibujándose alrededor. El fondo no es negro plano: lleva un rescoldo de oro muy bajo en el borde de arriba, que es lo que la hace leerse como una pieza puesta encima y no como un agujero. **Toda la carta mide contra una sola cifra**: el `font-size` de `.cb-tinta`, y todo lo que lleva dentro —cuerpos, márgenes, la raya de la rúbrica— va en `em`. Eso es lo que hace falta en una pantalla baja: la escena clava la carta a pantalla completa y ahí no hay a dónde seguir, de modo que lo que no cabe se pierde. Con una cifra, cabe: a partir de 760px de alto se aprieta el aire y por debajo de 700 baja también el tamaño base, atado al alto de la pantalla. Medido: sobra sitio en 412×915, 390×844, 375×667, 360×640 y 320×568. **La firma se invierte** (`filter: invert(1)` más `mix-blend-mode: screen`): el archivo es tinta oscura sobre transparente, hecho para papel blanco, y aquí el papel es ónix. El archivo sale de la firma en alta que pasó la clínica (1600×820, tinta negra sobre papel blanco, guardada en `imagenes/logos-originales/`): de su gris se saca el alfa con una rampa de 238 a 105 —un umbral duro deja el trazo dentado y el papel en una bruma gris—, se recorta a la tinta y se guarda en gris+alfa a 640 de ancho, la mitad de canales que RGBA para el mismo resultado, porque el color del trazo es plano. La anterior se sacaba del propio documento de 320×455 y se veía rota. Se escribe de izquierda a derecha con `--k-firma`, que es lo último que mueve la escena. Las `--k-*` van registradas con `@property` **y aquí sí heredan**, al revés que las del cine: allá el guion las escribía en la escena entera y cortar la herencia ahorraba recalcular doscientos elementos; aquí cada una se escribe ya en la pieza pequeña que la usa, y entre lo que cuelga de ella hay pseudo-elementos —la juntura, las ocho rayas de las escuadras— que tienen que leerla. La excepción es `--k-texto`, que se escribe en `.cb-tinta` y la usa esa misma pieza: sin herencia, cambiarla no obliga a repasar el estilo de lo que lleva dentro, y se funde **una capa** y no un párrafo tras otro. Su valor inicial es 1, el estado final: sin guion la carta se ve entera. Ojo con el título: la página lleva `pagina-clara` en el `<body>` y `.pagina-clara h2` pesa más que una clase sola, así que lo pintaba del negro de la piedra clara sobre el ónix; va como `.ns-carta .cb-tit`, que empata en peso y gana por ir después. **Y mientras la carta ocupa la pantalla, los tres botones flotantes se apartan** (`.con-carta` en el `<html>`, que pone nosotros-cine.js): le caían encima, y no hay sitio al que moverlos. Se van con un fundido y vuelven solos en cuanto el cierre se clava encima. | `.ns-carta*`, `.cb-*`, `.con-carta`, `--k-*` |
