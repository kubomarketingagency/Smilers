...
| `27-consentimiento.css` | **Va la última y la cargan las seis páginas.** El aviso de cookies vestido de la casa —negro de ónix, filo y botones de oro, la letra del sitio— a través de las variables de CookieConsent, con `:root #cc-main` para ganarle a su hoja, que llega después. En el teléfono pone aceptar y rechazar en una fila (la librería los apila con `!important`). Los dos enlaces legales del pie: «Política de privacidad» y el botón «Preferencias de cookies», que nace con `hidden` y solo enciende el guion cuando hay píxeles configurados. Y el **botón flotante de cookies** (`.btn-cookies`): abajo a la izquierda, porque a la derecha ya están WhatsApp y volver arriba; una galleta mordida de oro sobre ónix que al pasar por encima o al recibir el foco se abre hacia la derecha y enseña la palabra —en un teléfono, donde no hay «pasar por encima», se queda redondo—. Nace con `hidden` como el del pie, y se esconde mientras alguno de los dos paneles del aviso está abierto (`.show--consent`, `.show--preferences`, que la librería pone en el `<html>`), que ocupan esa misma esquina. | `#cc-main`, `.footer-legal`, `.enlace-cookies`, `.btn-cookies` |
| `28-legal.css` | **Solo la carga Privacidad.** El texto legal en una placa clara sobre el mármol, a medida de lectura (880px), con resumen, índice numerado y las tablas de cookies con su propio scroll lateral en el teléfono. Ojo: la fecha va como `.legal .legal__fecha`, porque `.legal p` pesa más que una clase sola y la ponía al cuerpo del texto. | `.legal*`, `.pagina-legal` |
...
- **`revision` en `AJUSTES`**: subirla vuelve a preguntar a todo el mundo. Hay
  ...
  hay que añadirlo a la tabla de cookies en dos sitios: los textos del aviso
