...
| `29-carta.css` | **Va la última y solo la carga Nosotros.** La carta de bienvenida: la tercera escena clavada de esa página, entre el cine y el cierre de cifras, montada igual que el cierre —300vh de envoltorio con un pin de `100lvh` dentro y un margen negativo de una pantalla, para engancharse a la última del cine sin dejar en medio una pantalla de negro quieto—. **Dos coreografías con el mismo progreso, y eso lo hace esta hoja, no el guion**: en pantalla ancha las dos mitades conviven a partes iguales —el director a la izquierda, la carta a la derecha— y se descubren por orden, las dos de izquierda a derecha (`clip-path: inset`), con la juntura de oro dibujándose de arriba abajo entre ellas; en el teléfono no caben las dos, así que se turnan —el retrato ocupa la pantalla con la frase encima y la carta sube desde abajo y lo tapa—, y la frase no necesita irse por su cuenta porque la tapa la propia carta. **Lo que se enseña es el contenido de la carta, no el papel.** Hubo una temporada en que aquí se rehacía el documento entero de la clínica —su banda negra con los arcos de oro, su columna de teléfonos, redes y direcciones, su raya vertical, su hoja blanca en proporción A4—. De ese documento se queda **lo que dice**; el resto se fue, y con la columna se fueron también sus enlaces (el teléfono, las redes, los mapas), que el pie lleva enteros: eran los mismos datos tres veces en la misma página. **Y tampoco es una tarjeta**: hubo después una plancha de ónix flotando en su mitad, con sombra y cuatro escuadras alrededor —una carta dentro de una caja dentro de una pantalla—. Ahora **la mitad es la carta**: blanco de canto a canto (`.ns-carta__hoja`), sin marco, sin sombra y sin nada detrás, con el texto centrado en ella y **dos** escuadras de oro (`--k-filo`) dibujándose en el canto derecho, arriba y abajo: son las que el documento llevaba a la izquierda, espejadas al lado que aquí queda libre, porque el izquierdo lo ocupa la juntura con el retrato. Sobre blanco **el oro es el hondo** (`--gradiente-oro-hondo`), que es la regla del 22 para piedra clara, y **el logo es el de mármol y no el de ónix**: son el mismo logo —oro con la muela de plata— y lo único que cambia es el renglón de «Dental Clinique», que en el de ónix va en blanco y sobre este fondo desaparecía. **La firma va tal cual, sin invertir** (`mix-blend-mode: multiply` para rematar los bordes del recorte): el archivo es tinta oscura sobre transparente, hecho para papel blanco, y ahora el papel vuelve a ser blanco. Sale de la firma en alta que pasó la clínica (1600×820, tinta negra sobre papel blanco, guardada en `imagenes/logos-originales/`): de su gris se saca el alfa con una rampa de 238 a 105 —un umbral duro deja el trazo dentado y el papel en una bruma gris—, se recorta a la tinta y se guarda en gris+alfa a 640 de ancho, la mitad de canales que RGBA para el mismo resultado, porque el color del trazo es plano. La anterior se sacaba del propio documento de 320×455 y se veía rota. Se escribe de izquierda a derecha con `--k-firma`, que es lo último que mueve la escena. **Toda la carta mide contra una sola cifra**: el `font-size` de `.cb-tinta`, y todo lo que lleva dentro —cuerpos, márgenes, la raya de la rúbrica— va en `em`. La escena clava la carta a pantalla completa y ahí no hay a dónde seguir, de modo que lo que no cabe se pierde; con una cifra, cabe. El techo es `--cb-tope`, atado al alto, **y va en dos tramos por ancho** porque lo que decide cuánto mide la carta no es solo el alto: cuanto más estrecha es la mitad, en más líneas cae el mismo texto. Medido, pide unos 40 em a partir de 1200px de ancho (`2vh`) y casi 46 por debajo, donde la columna baja de 450px (`1.85vh`); en el teléfono, `2.15vh` y `2.05vh` por debajo de 700 de alto, con dos escalones que aprietan el aire. Comprobado con regla en 1920×1080, 1440×900, 1366×768, 1280×720, 1200×700, 1024×768, 1024×640, 992×600, 412×915, 390×844, 375×667, 360×640 y 320×568: en todos sobra sitio por arriba y por abajo. **Ojo con la fila de la rejilla**: con la escena clavada lleva `grid-template-rows: 100%` y las dos mitades `min-height: 0`. Una fila implícita es `auto`, es decir, max-content: si la carta pedía más alto del que hay, la fila crecía, se salía del pin por abajo y el `overflow` se comía la firma **sin que nada lo dijera** —al medir, el hueco disponible salía siempre igual al que ocupaba el texto—. Las `--k-*` van registradas con `@property` **y aquí sí heredan**, al revés que las del cine: allá el guion las escribía en la escena entera y cortar la herencia ahorraba recalcular doscientos elementos; aquí cada una se escribe ya en la pieza pequeña que la usa, y entre lo que cuelga de ella hay pseudo-elementos —la juntura, las cuatro rayas de las escuadras— que tienen que leerla. La excepción es `--k-texto`, que se escribe en `.cb-tinta` y la usa esa misma pieza: sin herencia, cambiarla no obliga a repasar el estilo de lo que lleva dentro, y se funde **una capa** y no un párrafo tras otro. Su valor inicial es 1, el estado final: sin guion la carta se ve entera. Ojo con el título: la página lleva `pagina-clara` en el `<body>` y `.pagina-clara h2` pesa más que una clase sola, así que va como `.ns-carta .cb-tit`, que empata en peso y gana por ir después. **Y mientras la carta ocupa la pantalla, los tres botones flotantes se apartan** (`.con-carta` en el `<html>`, que pone nosotros-cine.js): le caían encima, y además son tres piezas oscuras sobre blanco. Se van con un fundido y vuelven solos en cuanto el cierre se clava encima. | `.ns-carta*`, `.cb-*`, `.con-carta`, `--k-*` |
...
| sin sufijo, `-720` | Las medianas, tal cual llegaron o recortadas, con su `-720` para el `srcset` cuando pasan de 900px. | El collage de Nosotros en la portada, el tríptico y los comparadores de Especialidades. |

**Las que llegaron pequeñas van donde se ven pequeñas.** Las del equipo delante de las pantallas (`tomografia`, `radiografia-implantes`, `radiografia-conductos`, `diseno-sonrisa` y `consulta-pantalla`) miden entre 565 y 916px de ancho: van en el collage, en las miniaturas y en los discos, donde se ven a su tamaño o más chicas, y en tres comparadores —Rehabilitación Oral, Endodoncia y Armonización Orofacial— porque son las fotos de esas especialidades. En esos comparadores, en escritorio, se estiran de 1,1 a 1,4 veces, y son lo único de la carpeta que no llega a 1:1 en una pantalla de ordenador. **La primera foto de la portada es la de las lámparas** (198 KB apaisada, 265 KB en vertical) y no la de la recepción con el jardín (371 KB): la alfombra y la pared verde son detalle puro, y la que se pinta primero tiene que pesar la mitad.

**Los discos de Testimonios son escenas de la clínica, no caras.** Cada ficha lleva la foto de la especialidad que cuenta —la radiografía, la tomografía, el sillón—, porque a una cita de muestra no se le pone la cara de un paciente de verdad. Las fotos de pacientes van en la Galería, en Sonrisas, que es donde se enseñan como lo que son.

**Con una excepción, y es de verdad.** El primer disco es un testimonio en vídeo —Anaela Guerrero, Reina de Pichincha, en la clínica—, así que ahí sí hay una cara: la suya, porque es ella quien cuenta lo que cuenta. Sus dos imágenes viven en `imagenes/testimonios/` y salen del fotograma que YouTube guarda del vídeo (`oardefault`, 1080×1920, el original en `imagenes/originales/`): `anaela-cuadro.webp` (720×720) es el recorte que va al atlas de la esfera, **tomado por arriba y no por el centro**, que ahí el centro son las piernas y la cabeza se quedaba fuera; `anaela-vertical.webp` (540×960) es el fotograma entero, que es lo que se ve dentro del marco 9:16 y lo que rellena, desenfocado, el resto del círculo. Se guardan aquí **y no se piden a YouTube** para que la página no haga ni una sola petición a su dominio hasta que alguien pulse el play (medido: cero).
...
| `testimonios-esfera.js` | La esfera WebGL de testimonios. Solo la carga `index.html`. **La esfera no se crea al abrir la página** sino cuando la sección queda a dos pantallas y media: el contexto WebGL, sus sombreadores y los dos atlas de catorce fotos eran lo más caro de la portada (tres tareas largas y catorce fotos compitiendo con la de arriba) para algo que está al final del recorrido. La sección sí se monta desde el principio —su clase `esfera-activa` y su alto `--alto-cine`—, que de eso depende dónde cae todo lo de detrás. Las fotos se descodifican fuera del hilo principal antes de pasar al atlas, el bucle de cada fotograma reutiliza sus matrices (antes creaba unas trescientas por fotograma y el recolector de basura se notaba en el teléfono) y, cuando la esfera está quieta y ya pintada, deja de calcular hasta que algo la mueve. **El tamaño del disco lo manda `encuadreSegunPantalla()`**: es la altura de escena que le cabe a la cámara, así que cuanto más alta, más campo y más pequeño sale el disco. En escritorio la caja mide 835 de lado y con 0.285 el disco salía de 425 —media caja—; con 0.235 llena su sitio, sin tocar la perspectiva ni mover la cámara. **`diametroDisco()` devuelve lo que mide el disco de delante en píxeles de pantalla**, y de ahí sale `--tst-disco`, que es lo que usa el círculo del vídeo para caer justo encima. Se calcula y no se mide a ojo: si mañana cambian `escala` o `encuadre`, el círculo cambia con ellos. La cuenta sigue lo que hace el sombreador, que no deja el disco plano —`radius * normalize(...)` lo convierte en un casquete de la esfera—, así que hay que proyectar el borde del casquete y no el radio suelto. **El círculo del vídeo solo se pone con la esfera quieta en el suyo** (`quietud > 0.55`), y al quitarlo se apaga el vídeo: si no, seguiría sonando detrás de otro testimonio. Mientras está puesto, el botón de antes/después se apaga, que sobre un vídeo no revela nada. |
...
| `video-testimonio.js` | **Solo lo carga la portada.** El testimonio en vídeo. En la página no hay ningún `<iframe>` escrito: hay un enlace a YouTube con el fotograma del vídeo detrás, y el `<iframe>` se crea al pulsarlo. Tres motivos, y los tres importan: el reproductor de YouTube pesa más que el resto de la portada junta y se descargaría aunque nadie le diera al play; deja cookies suyas en cuanto se carga, y este sitio pregunta antes de dejar ninguna que no sea necesaria (por eso además se pide a `youtube-nocookie.com`); y sin JS —o con el enlace en un sitio donde no cabe verse— el enlace sigue siendo un enlace y lleva al vídeo en YouTube. Monta el reproductor en el `[data-video-hueco]` que tenga encima el enlace pulsado, así que la misma pieza sirve para el círculo de la esfera y para la tarjeta de la lista. `apagar()` lo llama la escena de testimonios cuando la esfera se lleva ese círculo: apagar es **quitar** el iframe, no pausarlo, que mientras exista sigue siendo el reproductor con sus cookies y su sonido. |
| `consentimiento-aviso.js` | La configuración y los textos del aviso de CookieConsent. No va en ningún paquete de página: viaja con la librería en `paquetes/aviso-cookies.js`, que solo se pide cuando hay que preguntar. |
